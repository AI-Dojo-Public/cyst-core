...
my_custom_service = PassiveServiceConfig(
    ...
    local=True,
    access_level=AccessLevel.LIMITED,
    authentication_providers=[local_password_auth(ref="my_custom_service_auth_ref", id="my_custom_service_auth_id")],
    access_schemes=[
        AccessSchemeConfig(
            authentication_providers=["my_custom_service_auth_ref", remote_email_auth.ref],
            authorization_domain=AuthorizationDomainConfig(
                type=AuthorizationDomainType.LOCAL,
                authorizations=[
                    AuthorizationConfig("user1", AccessLevel.LIMITED),
...
connections = [
    ConnectionConfig(target.ref, 0, "router1", 1),
    ConnectionConfig(sso_server.ref, 0, "router1", 2),
    ConnectionConfig(email_server.ref, 0, "router1", 3)
]
